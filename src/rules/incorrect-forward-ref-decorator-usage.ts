        ...
        return {
            MethodDefinition(node) {
                ...
                for(const param of constructorParams){
                    const forwardRefDecorator = param.decorators.find(decorator => decorator.expression.arguments[0].callee?.name === 'forwardRef');
                    if(!forwardRefDecorator) continue;
                    
                    const forwardRefAnnotationType = getForwardRefDecoratorAnnotationType(param.parameter)
                    const forwardRefArgumentType = getForwardRefArgumentType(forwardRefDecorator.expression.arguments[0])
