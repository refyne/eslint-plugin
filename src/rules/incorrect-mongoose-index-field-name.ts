...
const IncorrectMongooseIndexFieldNameRule: Rule.RuleModule = {
    ...
    create(context) {
        ...
        function collectPropertyNames(node: any) {
            const classDeclarationNodes = node.body.filter(node => node.declaration?.type === AST_NODE_TYPES.ClassDeclaration);

            const classPropertyNodes = flatten(classDeclarationNodes.map(node => node.declaration.body.body));
            const propertyDefinitionNodes = classPropertyNodes.filter(node => node.type === AST_NODE_TYPES.PropertyDefinition);
        ...
        return {
            ...
            CallExpression(node) {
                if(node.callee?.property?.name !== 'index' || isEmpty(node.arguments)) return;

                // finding out indexes
                for (const property of node.arguments[0].properties) {
                    const indexFieldName = property.key.value ?? property.key.name;
                    const isIndexNestedField = isEmpty(property.key.name); // not supporting nested fields for now, TODO to take it up later
                    if (!isIndexNestedField && !propertyNames.includes(indexFieldName)) {
                        context.report({
                            node: node,
