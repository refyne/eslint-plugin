        ...
        return {
            Decorator: (node: any) => {
                if (node.expression.callee.name === 'Prop') {
                    ...
                    const parentTypeDeclaration = context.sourceCode.getText(
                        node.parent,
                    );

                    const optionalMatch = parentTypeDeclaration.match(/\?:/);
                    const propOptions =
                        getPropertiesFromArguments(propArguments);

                    const isMongooseTypeRequired =
                        propOptions['required'] ?? false;
                    const isTypeRequired = isEmpty(optionalMatch);

                    if (!isEqual(isMongooseTypeRequired, isTypeRequired)) {
                        const typeCode = replace(
